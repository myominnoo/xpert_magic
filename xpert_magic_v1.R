...
process_raw <- function(raw) {
  ...
  texts_included <- c("Patient ID|Analyte Name|HPV 16|HPV 18_45|P3,|P4,|P5,|SAC,|Start Time,")
  ...
  output[, -c(1:3)] <- apply(output[, -c(1:3)], 2, as.numeric)
  ...
  output[which(output$result == "Negative"), c(10:14)] <- 0
  output
}
...
sidebar <- sidebarPanel(
  ...
  hr(), 
  selectInput("vars", "Select columns to download:", choices = "", multiple = TRUE), 
...
server <- function(input, output, session) {
  ...
  df <- eventReactive(input$process, {
    ...
    for (i in 1:n) {
      ...
      raw <- suppressWarnings(process_raw(raw))
      vars <- names(raw)
